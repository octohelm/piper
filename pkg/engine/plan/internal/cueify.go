		...
		walkFields(tpe, func(i *fieldInfo) {
			...
			if i.inline {
				if t.Kind() == reflect.Map {
					_, _ = fmt.Fprintf(b, `[!~"\\$piper"]: %s`, c.toCueType(t.Elem(), opt{
						extra: i.cueExtra,
					}))
				}
				return
			...
			if len(i.attrs) > 0 {
				_, _ = fmt.Fprintf(b, " @piper(%s)", strings.Join(i.attrs, ","))
			}
...
func walkFields(s reflect.Type, each func(info *fieldInfo)) {
	for i := 0; i < s.NumField(); i++ {
		...
		piperTag, hasWagonTag := f.Tag.Lookup("piper")
